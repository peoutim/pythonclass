...
def wordSeek():
    for word in fileText:
        ...
        if word not in refText:
            ...
            candidateList = percentSameLocation(word, candidateList)
            #creates the candidateList by running through five funtions
...
def percentSameLocation(word, candidateList):
    #finds out if 60% of the letters are in the same direction
    #Augmented rule of own design
    candidateList1 = []
    for candidateWord in candidateList:
        if len(word) <= len(candidateWord):
            wordIndex = len(word)
        ...
        if wordIndex - wordIndexChange <= sameLocationCount \
           <= wordIndex + wordIndexChange:
            candidateList1.append(candidateWord)
        #adds the word if the sameLocation Count is within 60% of the wordIndex
    return candidateList1
